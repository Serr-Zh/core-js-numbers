 ...
 *
 * @param {number} _value1
 * @param {number} _value2
 * @return {number}
 *
 * @example:
 *   5, 5  => 5
...
function getAverage(value1, value2) {
  // Check if either value is greater than or equal to half of the maximum finite number
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sum = Math.sqrt(x1 * y1) + Math.sqrt(x2 * y2);
  return Math.acos(x1 * x2 + y1 * y2 - sum);
}
